	...
	for {
		...
		case <-timer.C:
			...
			err := mgr.with(func(c *faktory.Client) error {
				// mgr.Logger.Info(fmt.Sprintf("YVDEBUG Sending Faktory heartbeat >> %s", mgr.state))
				data, err := c.Beat(mgr.state)
				if err != nil {
					mgr.Logger.Warn(fmt.Sprintf("YVDEBUG Received Faktory heartbeat err >> %s", err.Error()))
				}
